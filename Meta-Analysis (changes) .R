...
library(readxl)
options(digits = 2)
extract <- read_excel("extract.xlsx", sheet = "Changes")
View(extract)
...
forest(respssL8, addpred = TRUE, showweights = TRUE,shade = "zebra",xlim = c(-19, 12),
...
forest(reshaml8, addpred = TRUE, showweights = TRUE,shade = "zebra",xlim = c(-18, 7), 
       ilab = cbind(haml8$`hm.L8.1`, haml8$`hv.L8.1`,haml8$n1,
                    haml8$`hm.L8.2`, haml8$`hv.L8.2`, haml8$n2),
       ilab.xpos = c(-15, -14, -13, -11.5, -10.5, -9.5), 
       cex = 0.8, header = "Author and Year")

text(c(-15, -14, -13, -11.5, -10.5, -9.5), reshaml8$k+2,
     c("Mean", "SD", "Total", "Mean", "SD", "Total"), cex = 0.8)
text(c(-14, -10.5), reshaml8$k+3, c("Intervention", "control"), cex = 0.8)
